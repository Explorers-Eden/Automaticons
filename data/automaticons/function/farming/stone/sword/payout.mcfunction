...
$execute unless score @s automaticons.tool.durability.current matches 0 run data modify entity @n[type=text_display,tag=$(uuid_0)$(uuid_1)$(uuid_2)$(uuid_3),tag=automaticons_durability_display] text set value '{"text":"Durability: $(durability)/64","color":"#c5bfbc","bold":false,"italic":false}'
