...
execute as @e[type=interaction,tag=ac_axe,tag=automaticon_interaction] if data entity @s attack at @s if score @e[type=armor_stand,tag=ac_axe,tag=automaticon,sort=nearest,limit=1] automaticons.tool.durability.current = @e[type=armor_stand,tag=ac_axe,tag=automaticon,sort=nearest,limit=1] automaticons.tool.durability.max on attacker run tag @s add ac_break
execute as @e[type=interaction,tag=ac_axe,tag=automaticon_interaction] if data entity @s attack at @s unless score @e[type=armor_stand,tag=ac_axe,tag=automaticon,sort=nearest,limit=1] automaticons.tool.durability.current = @e[type=armor_stand,tag=ac_axe,tag=automaticon,sort=nearest,limit=1] automaticons.tool.durability.max on attacker run title @p actionbar {"bold":false,"color":"#FF726E","italic":false,"text":"- Insufficient Durability -"}

execute as @s[tag=ac_break] run function automaticons:player_interactions/axe_kill with storage automaticons:kill 
